...
exports.pokazMape = void 0;

/* eslint-disable */
var pokazMape = function pokazMape(lokalizacje) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibHV0ZW5janVzeiIsImEiOiJjanpmNWV5bncwOHV5M2xqdzdkc3Bkb3hzIn0.MYE4I8ZXzpsTj1wxjiMf8w'; // indywidualny token dla konta/projektu

  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/lutencjusz/cjzf671s60wiu1cpgsne97aiu',
    // po utworzeniu własnego stylu mapy w studio kopiuje URL style
  ...
  var punkty = new mapboxgl.LngLatBounds(); // tworzy punkty przy użyciu biblioteki mapboxgl

  lokalizacje.forEach(function (lok) {
    ...
    }).setLngLat(lok.coordinates).setHTML("<p>Dzie\u0144 ".concat(lok.day, ": ").concat(lok.description, "</p>")) // opis popapu
    .addTo(map);
    punkty.extend(lok.coordinates); // rozszerza mapę o lokalizacje
  });
  map.fitBounds(punkty, {
    // wpasowuje mapę do punktów i ogranicza obszer, żeby punkty były widoczne w polu widoczności
    padding: {
      top: 200,
      bottom: 200,
      left: 100,
...
exports.pokazAlert = exports.ukryjAlert = void 0;

/* eslint-disable */
var ukryjAlert = function ukryjAlert() {
  var el = document.querySelector('.alert'); // znajduje element alert

  if (el) el.parentElement.removeChild(el); // usuwa element poprzez rodzica, który usuwa dziecko
};

exports.ukryjAlert = ukryjAlert;

var pokazAlert = function pokazAlert(typ, komunikat) {
  var czas = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 3;
  ...
  var znacznik = "<div class=\"alert alert--".concat(typ, "\">").concat(komunikat, "</div>");
  document.querySelector('body').insertAdjacentHTML('afterbegin', znacznik); // dodaje do body obiekt

  window.setTimeout(ukryjAlert, czas * 1000); // ukrywa alert po 3 sek.
};

exports.pokazAlert = pokazAlert;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (wynik.data.status === 'ok') {
              window.setTimeout(function () {
                // to raczej, żeby przetestować setTimeout 
          ...
          case 8:
            ...
            (0, _alert.pokazAlert)('error', 'Nieprawidłowe login lub hasło'); // jeżeli chcemy zobaczyć zawartość błedu to jest w err.response.data.message
    ...
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            wynik = _context2.sent;

            if (wynik.data.status === 'ok') {
              if (location.pathname === '/me') {
                // jeżeli po wylogowaniu się jest na /me to będzie błąd
          ...
          case 7:
            ...
            (0, _alert.pokazAlert)('error', 'Bład podczas wylogowania');
...
},{"axios":"../../node_modules/axios/index.js","./alert":"alert.js"}],"updateUstawienia.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.updateUstawienia = void 0;
...
function () {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'hasło' ? '/api/v1/uzytkownicy/updateMyPassword' : '/api/v1/uzytkownicy/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              // wykonuje podzapytanie do bazy
              method: 'PATCH',
              url: url,
              data: data // data jest obiketem
          ...
          case 4:
            ...
            if (wynik.data.status === 'ok') {
              (0, _alert.pokazAlert)('success', "Zmieniono Twoje ".concat(type, "!")); // location.assign('/me'); // nie uruhamiam odświerzenia, bo obcina czas pokazywania komunikatu
            }
          ...
          case 9:
            _context.prev = 9;
            _context.t0 = _context["catch"](0);
            (0, _alert.pokazAlert)('error', _context.t0.response.data.message); // jeżeli chcemy zobaczyć zawartość błedu to jest w err.response.data.message
  ...
  return function updateUstawienia(_x, _x2) {
    return _ref.apply(this, arguments);
  };
}();

exports.updateUstawienia = updateUstawienia;
...
function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(wycieczkaId) {
    var sesja;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("/api/v1/platnosci/checkout-sesja/".concat(wycieczkaId));
          ...
          case 8:
            ...
            (0, _alert.pokazAlert)('error', _context.t0);
  ...
  return function zakupWycieczke(_x) {
    return _ref.apply(this, arguments);
  };
}();

exports.zakupWycieczke = zakupWycieczke;
...
var _updateUstawienia = require("./updateUstawienia");
...
// elementy DOM
var zmiennaMap = document.getElementById('map');
var logowanieFormularz = document.querySelector('.form--login'); // czy został wybrany przycisk klasy .form 

var wylogowanie = document.querySelector('.nav__el--logout'); // czy został wybrany przycisk klasy .nav__el--logout   

var uzytkownikUstawieniaFormularz = document.querySelector('.form-user-data');
var uzytkownikHasloFormularz = document.querySelector('.form-user-password');
var zakupPrzycisk = document.getElementById('zarezerwuj-wycieczke');
var alarmKomunikat = document.querySelector('body').dataset.alarm; // delegacje

if (zmiennaMap) {
  var lokalizacje = JSON.parse(zmiennaMap.dataset.locations); // pobiera dane z elementu zmiennaMap jako string i konwertuje je na obiekt JSON

  (0, _mapbox.pokazMape)(lokalizacje);
}

if (logowanieFormularz) {
  logowanieFormularz.addEventListener('submit', function (el) {
    // podkreśla document bo eslint nie zna js kodu
    el.preventDefault(); // zapobiega uruchamianiu innych linków

    var email = document.getElementById('email').value; // wcześniej tych zmiennych nie widać

    var password = document.getElementById('password').value; // wcześniej tych zmiennych nie widać
...
if (wylogowanie) {
  wylogowanie.addEventListener('click', _login.logout);
}

if (uzytkownikUstawieniaFormularz) {
  uzytkownikUstawieniaFormularz.addEventListener('submit', function (el) {
    // podkreśla document bo eslint nie zna js kodu
    el.preventDefault(); //  zapobiega uruchamianiu innych linków

    var formularz = new FormData(); // tworzy dormularz z danymi
    ...
    */
    ...
    (0, _updateUstawienia.updateUstawienia)(formularz, 'ustawienia'); // name i hasło muszą być przekazywane jako obiekt, wczęsniej było {name, email}
  });
}

if (uzytkownikHasloFormularz) {
  uzytkownikHasloFormularz.addEventListener('submit',
  /*#__PURE__*/
  function () {
    var _ref = _asyncToGenerator(
    /*#__PURE__*/
    regeneratorRuntime.mark(function _callee(el) {
      var b, passwordCurrent, password, passwordConfirm;
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              b = document.querySelector('.btn--save-password');
              document.querySelector('.btn--save-password').textContent = 'Zapisywanie...'; // zmiana napisu na przycisku

              passwordCurrent = document.getElementById('password-current').value; // wcześniej tych zmiennych nie widać

              password = document.getElementById('password').value; // wcześniej tych zmiennych nie widać

              passwordConfirm = document.getElementById('password-confirm').value; // wcześniej tych zmiennych nie widać

              _context.next = 8;
              return (0, _updateUstawienia.updateUstawienia)({
            ...
            case 8:
              // name i hasło muszą być przekazywane jako obiekt
